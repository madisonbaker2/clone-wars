...
function placeholderBackground(ctx, width, height, time) { ctx.save(); ctx.fillStyle = '#6aaed6'; ctx.fillRect(0, 0, width, height); ctx.restore(); }
function placeholderGround(ctx, width, height, groundHeight, offset) { ctx.save(); ctx.fillStyle = '#6b8e23'; ctx.fillRect(0, height - groundHeight, width, groundHeight); ctx.restore(); }
function placeholderBird(ctx, x, y, size, velocity) { ctx.save(); ctx.fillStyle = '#f5c542'; ctx.fillRect(x - size / 2, y - size / 2, size, size); ctx.restore(); }
function placeholderPipe(ctx, x, gapTop, gapBottom, pipeWidth, height) { ctx.save(); ctx.fillStyle = '#4d8b31'; ctx.fillRect(x, 0, pipeWidth, gapTop); ctx.fillRect(x, gapBottom, pipeWidth, height - gapBottom); ctx.restore(); }
...
let state = 'ready', currentMode = 'normal', y = (CONFIG.canvasHeight - CONFIG.groundHeight) / 2, velocity = 0, pipes = [], pipesMade = 0, groundOffset = 0, lastGapTop = (CONFIG.canvasHeight - CONFIG.groundHeight - CONFIG.pipeGap) / 2, score = 0, bestScore = 0, checkpoint = 0, secondsSinceCrash = 0, lastTime = null;
...
function showReady() { overlay.hidden = false; overlayTitle.textContent = CONFIG.title; overlayText.textContent = 'Press Space, click or tap to start.\n' + soundLine(); sr.textContent = CONFIG.title + '. Ready to play.'; }
function showGameOver() { overlay.hidden = false; overlayTitle.textContent = 'Game over'; const lines = ['Score ' + score + '   ·   Best ' + bestScore, 'Press Space, click or tap to play again.']; if (checkpoint > 0) lines.push('Next game starts at checkpoint ' + checkpoint + '.'); lines.push(soundLine()); overlayText.textContent = lines.join('\n'); }
...
function touchesPipe(pipe) { const birdLeft = birdX - CONFIG.birdSize / 2, birdRight = birdX + CONFIG.birdSize / 2, birdTop = y - CONFIG.birdSize / 2, birdBottom = y + CONFIG.birdSize / 2; return birdRight > pipe.x && birdLeft < pipe.x + CONFIG.pipeWidth && (birdTop <= pipe.gapTop || birdBottom >= pipe.gapBottom); }
function drawScore() { ctx.save(); ctx.textAlign = 'center'; ctx.font = '800 40px system-ui, sans-serif'; ctx.lineWidth = 6; ctx.strokeStyle = 'rgba(24, 59, 59, 0.9)'; ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 58); ctx.fillStyle = '#ffffff'; ctx.fillText(String(score), CONFIG.canvasWidth / 2, 58); ctx.restore(); }
function frame(now) { const seconds = lastTime === null ? 0 : Math.min(0.05, Math.max(0, (now - lastTime) / 1000)); lastTime = now; if (state === 'playing') { const mode = CONFIG.fix === 'easy-mode' ? CONFIG.modes[currentMode] : CONFIG; const pipeGap = mode.pipeGap; const pipeSpeed = mode.pipeSpeed * (CONFIG.fix === 'gentle-start' && score < 3 ? 0.75 : 1); velocity += CONFIG.gravity * seconds; y += velocity * seconds; groundOffset += pipeSpeed * seconds; if (pipes.length === 0) addPipe(pipeGap); else if (pipes[pipes.length - 1].x <= CONFIG.canvasWidth - CONFIG.pipeSpacing) addPipe(pipeGap); for (const pipe of pipes) { pipe.x -= pipeSpeed * seconds; if (!pipe.scored && pipe.x + CONFIG.pipeWidth < birdX) { pipe.scored = true; score += 1; play('score'); } } pipes = pipes.filter((pipe) => pipe.x + CONFIG.pipeWidth > 0); if (y + CONFIG.birdSize / 2 >= CONFIG.canvasHeight - CONFIG.groundHeight || y - CONFIG.birdSize / 2 <= 0 || pipes.some(touchesPipe)) crash(); } else if (state === 'gameover') secondsSinceCrash += seconds; ctx.clearRect(0, 0, CONFIG.canvasWidth, CONFIG.canvasHeight); drawBackground(ctx, CONFIG.canvasWidth, CONFIG.canvasHeight, reduceMotion ? 0 : now / 1000); for (const pipe of pipes) drawPipe(ctx, pipe.x, pipe.gapTop, pipe.gapBottom, CONFIG.pipeWidth, CONFIG.canvasHeight - CONFIG.groundHeight); drawGround(ctx, CONFIG.canvasWidth, CONFIG.canvasHeight, CONFIG.groundHeight, groundOffset); drawBird(ctx, birdX, y, CONFIG.birdSize, velocity); if (state === 'playing') drawScore(); requestAnimationFrame(frame); }
showReady();
